...
def substitution(R):
    ...
    for i in range(8):
        iData = data >> (7 - i) * 6
        h = ((iData >> 5 & 1) << 1) | (iData & 1)
        l = (iData >> 1) & 0xF
        sData = sData | S[i][h * 16 + l]
        sData = sData << 4
    return sData >> 4


def F(R, k):
    expandR = replace(R, E, 32)
    expandR = expandR ^ k
    subR = substitution(expandR)
    return replace(subR, P, 32)
...
def DES_encrypt(data, key):
    cdata = replace(data, IP, 64)
    keyList = init_key(key)
    L = cdata >> 32
    R = cdata & 0xFFFFFFFF
    for i in range(16):
        k = keyList[i]
